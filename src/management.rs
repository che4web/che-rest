...
use che_orm::{Database, FieldType, Schema, SqliteBackend, diff_schemas, sqlite_migration_sql};
...
fn is_json_format(format: &str) -> ManageResult<bool> {
    match format {
        "text" => Ok(false),
        "json" => Ok(true),
        _ => Err(format!("unsupported format: {format}; expected text or json").into()),
    }
}
...
impl Management {
    ...
    async fn run_from(self, cli: Cli) -> ManageResult<()> {
        match cli.command {
            ...
            Command::Makemigrations {
                ...
                apps_dir,
                name,
            } => self.makemigrations(app.as_deref(), apps_dir, &name, &format)?,
            Command::Inspect {
                format,
                apps_dir,
                config,
            ...
            Command::Migrate {
                ...
                self.migrate(app.as_deref(), apps_dir, config, database_url, &format)
                    .await?
            }
            Command::GenerateTs { out, format } => self.generate_ts(out, &format)?,
            Command::GenerateAdmin {
                out,
                format,
                templates_dir,
                force,
            } => self.generate_admin(out, templates_dir, force, &format)?,
    ...
    fn makemigrations(
        ...
        let json = is_json_format(format)?;
        let app_names = app
            .map(|app| vec![app.to_string()])
            .unwrap_or_else(|| self.apps.names().map(str::to_string).collect());
        let results = app_names
            .iter()
            .map(|app| self.makemigrations_app(app, &apps_dir, name))
        ...
        if json {
            ...
            for result in results {
                if let Some(path) = result["migration"].as_str() {
                    println!("Created {path}");
                } else {
                    println!("No schema changes detected for app {}", result["app"]);
                }
            }
        }
        Ok(())
    ...
    async fn migrate(
        ...
        for app in app_names {
            ...
            let applied = apply_app_migrations(&db, &apps_dir, &app).await?;
            results.push(serde_json::json!({
                "app": app,
                "status": if applied.is_empty() { "unchanged" } else { "applied" },
                "migrations": applied,
            }));
        }
        ...
        if json {
            ...
            for result in results {
                for migration in result["migrations"].as_array().into_iter().flatten() {
                    println!("Applied {}: {}", result["app"], migration);
                }
            }
        }
    ...
    fn generate_admin(
        ...
        write_generated_file(&out.join("src/generated/auth.ts"), auth_ts())?;
        write_generated_file(
        ...
        if json {
            let mut files = ADMIN_TEMPLATES
                .iter()
                .map(|template| template.path.to_string())
                .collect::<Vec<_>>();
            files.extend(admin_pages.iter().flat_map(|page| {
                [
                    format!("src/admin/pages/{}List.vue", page.base_name),
                    format!("src/admin/pages/{}Form.vue", page.base_name),
                ]
            }));
            files.extend([
                "src/admin/generated/adminSchema.ts".to_string(),
                "src/admin/generated/adminRoutes.ts".to_string(),
                "src/admin/adminSchema.ts".to_string(),
                "src/generated/api_client.ts".to_string(),
                "src/generated/channels.ts".to_string(),
                "src/generated/models.ts".to_string(),
                "src/generated/api.ts".to_string(),
                "src/generated/auth.ts".to_string(),
                "src/generated/useModelList.ts".to_string(),
                "src/generated/useModelItem.ts".to_string(),
            ]);
            println!(
                "{}",
                serde_json::to_string_pretty(&serde_json::json!({
                    "command": "generate-admin",
                    "status": if force { "generated" } else { "updated" },
                    "output": out,
                    "files": files,
                }))?
            );
...
async fn createsuperuser(
    ...
    let existing = db
        .query::<auth::models::User>()
        .filter(auth::models::UserFields::USERNAME.eq(username))
        .limit(1)
        .all()
        .await?;
...
fn models_ts(endpoints: &[ApiEndpoint]) -> String {
    ...
    let endpoint_models = endpoints
        .iter()
        .map(|endpoint| endpoint.model_name.as_str())
        .collect::<BTreeSet<_>>();
    let related_models = endpoints
        .iter()
        .flat_map(|endpoint| endpoint.fields.iter())
        .filter_map(|field| field.related_model.as_deref())
        .filter(|model| !endpoint_models.contains(model))
        .collect::<BTreeSet<_>>();
