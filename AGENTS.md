...
- Installing `che_rest::channels::module()` adds `/api/ws/` for declared public signals. CRUD viewsets declare authenticated `<resource>.created`, `<resource>.updated`, and `<resource>.deleted` signals automatically and publish `{ "id": ... }` payloads; nested paths use dotted names like `auth.users.created`. Use `ModuleContext::signal(...)` for custom public signals. `AppState::app_channels()` remains internal-only.
