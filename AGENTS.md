...
- Installing `che_rest::channels::module()` adds `/api/ws/` for declared public signals. CRUD viewsets declare authenticated `<resource>.created`, `<resource>.updated`, and `<resource>.deleted` signals automatically; use `ModuleContext::signal(...)` for custom public signals. `AppState::app_channels()` remains internal-only.
