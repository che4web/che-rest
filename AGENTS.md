...
- `generate-ts` writes `api_client.ts`, `models.ts`, `api.ts`, `useModelList.ts`, and `useModelItem.ts`; `generate-admin` writes Vue admin files. These outputs are produced from installed app metadata, not by scanning source files.
